...
#### Método 1: Interface de Chat

1.  **Prompt de Sistema Inicial:**
    *   No início de uma nova conversa com o Claude, copie e cole **todo o conteúdo** do arquivo `1- System Prompt.md`. É recomendável usar delimitadores claros, por exemplo:
        ```
        <SYSTEM_PROMPT_ENG_JURIDICO>
        [Cole aqui o conteúdo completo do arquivo 1- System Prompt.md]
        </SYSTEM_PROMPT_ENG_JURIDICO>

        A partir de agora, você atuará como o Engenheiro de Prompts Jurídicos conforme definido acima. A sua base de conhecimento adicional será fornecida a seguir.
        ```
2.  **Disponibilizar a Base de Conhecimento:**
    *   **Upload de Arquivos:** Claude permite o upload de arquivos diretamente na interface de chat. Faça o upload dos arquivos `2- Preferencias de Formatacao Prompt.md`, `3- Modulo_QA.md` e `4- Templates_e_Exemplos.md`. Após o upload, instrua o Claude a considerar o conteúdo desses arquivos como parte integral de sua base de conhecimento para a sessão.
        ```
        Considere o conteúdo dos arquivos que acabei de enviar ('2- Preferencias de Formatacao Prompt.md', '3- Modulo_QA.md', '4- Templates_e_Exemplos.md') como sua base de conhecimento ativa para todas as minhas solicitações subsequentes nesta conversa.
        ```
    *   **Colar Conteúdo:** Se o upload não for viável ou para garantir que o conteúdo esteja sempre no contexto imediato (especialmente para modelos com janelas de contexto muito grandes), você pode colar o conteúdo dos arquivos `2`, `3` e `4` diretamente no chat, similar ao Método 2 do ChatGPT, usando delimitadores.

#### Método 2: API da Anthropic

1.  **System Prompt:**
    *   Utilize o conteúdo do arquivo `1- System Prompt.md` no parâmetro `system` da sua chamada de API.
2.  **Fornecendo Contexto Adicional:**
    *   Concatene o conteúdo dos arquivos `2`, `3` e `4` e inclua-o como parte do primeiro `user message` (turno do usuário) na sua conversa, antes da primeira solicitação efetiva do usuário.
    *   Para aplicações mais avançadas, considere estratégias de RAG.

---

### **Gemini (Google)**

#### Método 1: Google AI Studio (ou interfaces similares)

1.  **Instrução de Sistema/Contexto:**
    *   No Google AI Studio, você pode configurar o comportamento do modelo fornecendo instruções no início do chat. Copie e cole **todo o conteúdo** do arquivo `1- System Prompt.md` nesta área de instrução ou no primeiro turno do chat.
2.  **Disponibilizar a Base de Conhecimento:**
    *   O Google AI Studio e a API Gemini suportam o envio de arquivos ou dados textuais que podem ser usados como contexto.
    *   **Upload de Arquivos (via API ou se a UI suportar):** Se estiver usando a API Gemini, você pode enviar o conteúdo dos arquivos `2`, `3` e `4` como parte da sua solicitação (ex: usando `parts` com `fileData` ou `text`). Se a interface do AI Studio permitir upload direto para o contexto, utilize essa funcionalidade.
    *   **Colar Conteúdo:** Na interface do AI Studio, você pode colar o conteúdo dos arquivos `2`, `3` e `4` no início da conversa, após o prompt do sistema, usando delimitadores claros.
        ```
        <BASE_DE_CONHECIMENTO_ADICIONAL>
...
1.  **System Instructions / Model Tuning:**
    *   Dependendo da forma como você está usando a API Gemini (ex: Vertex AI), você pode ter opções para definir instruções de sistema ou até mesmo realizar um leve ajuste (tuning) do modelo com o `1- System Prompt.md` como base.
2.  **Fornecendo Conteúdo Programaticamente:**
    *   Envie o conteúdo dos arquivos `1`, `2`, `3` e `4` como parte do objeto `contents` na sua solicitação à API. O conteúdo do `1- System Prompt.md` deve idealmente ser configurado como uma instrução de sistema, se a API permitir de forma distinta, ou como o primeiro elemento em `contents`. Os demais arquivos podem ser passados como `parts` textuais subsequentes ou como `tools` do tipo `CODE_INTERPRETER` se precisar que o modelo "leia" os arquivos.
    *   Para contextos muito longos, pode ser necessário concatenar os conteúdos e enviá-los como um único bloco de texto ou usar chamadas multimodais se estiverem em formatos como PDF (embora os arquivos `.md` sejam texto puro).

---

**Considerações Gerais:**

*   **Janela de Contexto:** Esteja ciente das limitações da janela de contexto do modelo específico que você está usando. Para modelos com janelas menores, colar o conteúdo completo de todos os arquivos `.md` em cada sessão pode não ser viável. Nesses casos, priorize o `1- System Prompt.md` e forneça seções relevantes dos outros arquivos conforme necessário para a tarefa. O uso de Custom GPTs ou APIs com RAG é preferível para gerenciar grandes volumes de conhecimento.
*   **Atualizações da Plataforma:** As interfaces e funcionalidades das plataformas de IA evoluem rapidamente. Consulte sempre a documentação oficial mais recente da OpenAI, Anthropic e Google para obter as instruções mais precisas sobre como configurar prompts de sistema e fornecer dados de contexto.
*   **Teste:** Após a configuração, realize testes com os diferentes modos (`#prompt`, `#qa`, `#learn`, `#edge`) para garantir que o Engenheiro de Prompts Jurídicos está acessando e utilizando corretamente todas as partes de sua base de conhecimento.
